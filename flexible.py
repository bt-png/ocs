...
def CatenarySag_Flexible(WireRun, L_Weight, L_Tension, L_SpanLoading, L_DesignHA, L_DesignBase):
    P_DiscreteLoad_CW, STA_DiscreteLoad_CW, P_DiscreteLoad_MW, STA_DiscreteLoad_MW = np.array([L_SpanLoading.iloc[:,i].tolist() for i in (2,1,3,1)])
    ...
    P_SpanWeight = GenFun.AddDiscreteLoadstoSpan(P_SpanWeight, 
                                                Stationing,
                                                np.array([yMultiplier/xMultiplier * x for x in P_DiscreteLoad_CW]),
                                                np.array([xMultiplier * x for x in STA_DiscreteLoad_CW]),
                                                xRound)
    ...
    P_SpanWeight_MW = GenFun.AddDiscreteLoadstoSpan(P_SpanWeight_MW,
                                                    Stationing,
                                                    np.array([yMultiplier/xMultiplier * x for x in P_DiscreteLoad_MW]), 
                                                    np.array([xMultiplier * x for x in STA_DiscreteLoad_MW]),
                                                    xRound)
