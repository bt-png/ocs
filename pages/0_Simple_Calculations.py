...
with conductor:
    ...
    with cdd1:
        ...
        with st.expander('Wind Data'):
            ...
            if _c_iced_radius == 0:
                 st.write('Wind Load = ', str(_c_windload_bare), '(pounds per square feet)')
            else:
                st.write('Wind Load: (pounds per foot)')
                st.write('Bare conductor =' , str(_c_windload_bare))
                st.write('Iced conductor =' , str(_c_windload_iced))
        with st.expander('Cable Tension'):
            ...
            _c_chk_wind = st.checkbox(label='Include Wind Pressure in Final Tension Condition', value=False)
            _c_tension_final = GenFun.ConductorTension(_c_E, _c_alpha, _c_tension,_c_weight+_c_chk_ice*_c_iced_weight,(_c_weight*(100-_c_section_loss)/100)+_c_iced_weight,_c_temp_diff,_c_equiv_span,_c_area, _c_area*(100-_c_section_loss)/100)
        ...
        with st.expander('Sag & Wind Blowoff', expanded=True):
            ...
            df = pd.concat([dfb, dfc.iloc[::-1]], ignore_index=True)
            _ft, _in , _ftin = GenFun.FtIn(max(_c_blowoff)/12)
            st.write('Max Blow-off = ', _ftin)
            plotBlowOff(dfb, dfc)
            st.write('Section View at Mid-Span')
