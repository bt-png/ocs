...
def CatenarySag_FlexibleHA_Iterative(WireRun, L_WeightChange, L_Tension, L_DesignHA, L_DesignBase, L_StaticCWUplift, ORIGINALDESIGN):
    ...
    while loops < 25:
        ...
        while subLoop1 < 25:
            ...
            Cycle_CWELDiff = GenFun.CWSupportELDifference(GenFun.RoundVal(Stationing,xRound), ORIGINALDESIGN.get('LoadedSag'),
                                                          Cycle_LoadedSag, GenFun.RoundVal(np.array(WR['STA']),xRound))
            Cycle_CWSupportReaction = GenFun.SteadyArm_VerticalResettingForce(RadialLoad, SteadyArmLength, Cycle_CWELDiff/yMultiplier)
...
def CatenaryElasticity_FlexibleHA(WireRun, L_WeightDiff, L_Tension, L_DesignHA, L_DesignBase, UpliftLoad, 
    ...
    for UpliftSTA in STAVal:
        print('Elasticity with uplift at STA=', UpliftSTA)
