...
def WireRun_df(df, STARound):
    ...
    val['STA'] = val['STA']=(val['STA']/STARound).round(0)*STARound
...
def SteadyArm_VerticalResettingForce(StaticRadialLoad, SteadyArmLength, ELDiff):
    return (StaticRadialLoad*ELDiff)/SteadyArmLength
...
def SupportLoadRight(STAList, LoadList, TensionList, HangerStationing, HangerElevation, STARound):
    ...
    SupportLoad = HangerStationing*0
    for i in range(0,len(HangerStationing)-1):
        ...
        SupportLoad[i] = (np.nansum(P*a)-H*h)/l
    SupportLoad = np.roll(SupportLoad,1)
    SupportLoad[0] = 0
    return SupportLoad
...
def SupportLoadLeft(STAList, LoadList, HangerStationing, HangerSupportLoadRight, STARound):
    ...
    for i in range(0,len(HangerStationing)-1):
        Step = (STAList>=HangerStationing[i]) * (STAList < HangerStationing[i+1])
        P = Step*LoadList
        SupportLoad[i] = np.nansum(P) - HangerSupportLoadRight[i+1]
    return SupportLoad
